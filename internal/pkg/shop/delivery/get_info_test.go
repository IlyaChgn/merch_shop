...
func TestGetInfo(t *testing.T) {
	...
	err = json.NewDecoder(authWriter.Body).Decode(&tokenStruct)
	if err != nil {
		t.Fatal(err)
	}
	...
	shopReq, err := http.NewRequest("GET", "/api/buy", nil)
	if err != nil {
		t.Fatalf("failed to create buy item request %v", err)
	}
	shopReq = mux.SetURLVars(shopReq, map[string]string{"item": "cup"})
	shopReq.Header.Set("Authorization", tokenStruct.Token)

	shopWriter := httptest.NewRecorder()
	shopHandle := http.HandlerFunc(shopHandler.BuyMerch)
	...
	infoReq, err := http.NewRequest("GET", "/api/info", nil)
	if err != nil {
		t.Fatalf("failed to create info request %v", err)
	}
	infoReq.Header.Set("Authorization", tokenStruct.Token)

	infoWriter := httptest.NewRecorder()
	infoHandle := http.HandlerFunc(shopHandler.GetInfo)

	infoHandle.ServeHTTP(shopWriter, shopReq)
	if infoWriter.Code != responses.StatusOk {
		t.Errorf("Expected status OK, got %v", shopWriter.Code)
	}
}
